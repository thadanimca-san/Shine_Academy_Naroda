...
/// Generates randomized Measures of Central Tendency problems across all
/// three data shapes. Difficulty controls which shape is used: 1-2 ->
/// individual series (easiest to compute by hand), 3 -> discrete series,
/// 4-5 -> continuous series (hardest, requires the interpolation
/// formulas for Median and Mode).
class CentralTendencyGenerator {
  CentralTendencyGenerator._();

  static final Random _rand = Random();

  static GeneratedCentralTendencyProblem generate({
    ...
    final result = CentralTendencySolver.solve(series);

    return GeneratedCentralTendencyProblem(
      id: 'CT-${DateTime.now().millisecondsSinceEpoch}-${_rand.nextInt(9999)}',
      board: board,
  ...
  static DataSeries _generateIndividual() {
    ...
    final values = List.generate(count, (_) => (10 + _rand.nextInt(90)).toDouble());
  ...
  static DataSeries _generateDiscrete() {
    ...
    while (usedValues.length < distinctCount) {
      usedValues.add((1 + _rand.nextInt(20)).toDouble());
    }
...
class GeneratedCentralTendencyProblem {
  final String id;
  final Board board;
  final int schoolClass;
  final int difficulty;
  final DataSeries series;
  final CentralTendencyResult result;

  const GeneratedCentralTendencyProblem({
    required this.id,
    required this.board,
